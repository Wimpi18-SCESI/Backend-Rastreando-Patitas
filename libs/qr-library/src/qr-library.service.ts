...
export class QrLibraryService {
	...
	getQrMatrix(message : string) {
		...
		const aux = bitData.mode + bitData.count + bitData.data + bitData.terminator;
		for (let i = 0; i < (272 - aux.length)/8; i++) {
			if (i%2==0)
				bitData.bytePadding += "11101100";
			else 
				bitData.bytePadding += "00010001";
		}
	...
	private convertToDataCodewordBytes(binaryMessage:string){
		for (let i = 0; i < binaryMessage.length/8; i++) {
			...
			const codeword:string = parseInt(binaryMessage.substring(i*8, i*8+8), 2).toString(16);
